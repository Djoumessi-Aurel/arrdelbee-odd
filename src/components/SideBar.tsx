                    ...
                    <div className="w-full h-full mb-2 flex flex-col justify-start items-center gap-2 border-b border-grayToneStroke">
                        {
                            MENU_LIST.map((item:MenuItemType, index:number)=>{
                                const ISACTIVE = decodeURI(PATHNAME) == item.href || decodeURI(PATHNAME).replaceAll("-"," ").trim().toLowerCase().includes(item.label.trim().toLowerCase()) || (PATH_ARRAY[PATH_ARRAY.length-1] === "dashboard" && item.label==="Accueil")
